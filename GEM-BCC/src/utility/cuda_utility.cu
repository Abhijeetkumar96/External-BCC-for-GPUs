...
void kernelPrintEdgeList(int* d_u_arr, int* d_v_arr, long numEdges, cudaStream_t stream) {
    print_edge_list<<<1, 1, 0, stream>>>(d_u_arr, d_v_arr, numEdges);
    CUDA_CHECK(cudaStreamSynchronize(stream), "Failed to synchronize print_edge_list");
}

void kernelPrintArray(int* d_arr, int numItems, cudaStream_t stream) {
    print_array<<<1, 1, 0, stream>>>(d_arr, numItems);
    CUDA_CHECK(cudaStreamSynchronize(stream), "Failed to synchronize print_array");
}

void kernelPrintCSRUnweighted(long* d_rowPtr, int* d_colIdx, int numRows, cudaStream_t stream) {
    print_csr_unweighted<<<1, 1, 0, stream>>>(d_rowPtr, d_colIdx, numRows);
