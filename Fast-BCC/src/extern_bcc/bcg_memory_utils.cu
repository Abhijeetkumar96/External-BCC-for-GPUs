...
GPU_BCG::GPU_BCG(int vertices, long num_edges, long _batchSize) : numVert(vertices), orig_numVert(vertices), orig_numEdges(num_edges), batchSize(_batchSize) {
    ...
    std::cout << "Allocation Size: " << batchSize * sizeof(uint64_t) << std::endl;

    size_t batch_alloc = batchSize * sizeof(uint64_t);

    std::cout << "Allocation Size: " << static_cast<double>(batch_alloc) / (1024 * 1024) << " MB" << std::endl;
    ...
    CUDA_CHECK(cudaMalloc(&d_fg, sizeof(int) * numVert), "Failed to allocate memory for d_fg");
